...
type Socks5UDPClient struct {
	Client              *Socks5Client // 底层 SOCKS5 客户端
	serverTCPConn       net.Conn      // 与 SOCKS5 服务器的 TCP 控制连接
	localUDPConn        net.UDPConn   // 本地 UDP 监听，用于收发 SOCKS5 UDP 数据报
	socks5ServerUDPAddr *net.UDPAddr  // SOCKS5 服务器返回的 UDP 绑定地址
	once                sync.Once     // 确保只关闭一次

	// 用于等待 UDP 转发 goroutine 退出
	wg sync.WaitGroup
	// 用于通知 UDP 转发 goroutine 退出
	ctx    context.Context
	cancel context.CancelFunc
}

// DialUDP 方法用于 UDP ASSOCIATE 命令
// host/port 参数通常是 0.0.0.0:0，实际目标在每个 UDP 包头中
func (c *Socks5Client) DialUDP(network, address string) (*Socks5UDPClient, error) {
	if network != "udp" && network != "udp4" && network != "udp6" {
		return nil, fmt.Errorf("unsupported network type: %s, only udp/udp4/udp6 is supported by SOCKS5 UDP ASSOCIATE", network)
	}

	// UDP ASSOCIATE 协议要求客户端先建立 TCP 连接，然后发送 UDP ASSOCIATE 请求
	// 这个 TCP 连接用于控制，其生命周期与 UDP 关联一致。
	serverTCPConn, err := net.Dial("tcp", c.Socks5Addr)
	if err != nil {
		return nil, fmt.Errorf("dial SOCKS5 server %s for UDP ASSOCIATE error: %w", c.Socks5Addr, err)
	}
	...
	// SOCKS5 握手和认证
	err = c.socks5Handshake(serverTCPConn)
	if err != nil {
		serverTCPConn.Close()
		return nil, fmt.Errorf("SOCKS5 handshake for UDP ASSOCIATE failed: %w", err)
	}

	// 发送 UDP ASSOCIATE 请求 (DST.ADDR 和 DST.PORT 通常是 0.0.0.0:0)
	// 如果 address 为空或无效，使用 0.0.0.0:0
	reqHost, reqPort := "0.0.0.0", 0
	if address != "" {
		h, pStr, splitErr := net.SplitHostPort(address)
		if splitErr == nil {
			p, atoiErr := strconv.Atoi(pStr)
			if atoiErr == nil {
				reqHost = h
				reqPort = p
			}
	...
	_, err = c.sendSocks5RequestHeader(serverTCPConn, CMD_UDP_ASSOCIATE, reqHost, reqPort)
	if err != nil {
		serverTCPConn.Close()
		return nil, fmt.Errorf("send SOCKS5 UDP ASSOCIATE request error: %w", err)
	}

	// 读取 UDP ASSOCIATE 响应，获取服务器绑定的 UDP 地址和端口
	bindAddr, err := c.readSocks5Response(serverTCPConn) // This bindAddr is the server's UDP binding
	if err != nil {
		serverTCPConn.Close()
		return nil, fmt.Errorf("read SOCKS5 UDP ASSOCIATE response error: %w", err)
	}

	//log.Printf("SOCKS5 server bound UDP to %s for association.", bindAddr.String())

	// 客户端本地也需要一个 UDP socket 来发送和接收 SOCKS5 UDP 数据报
	// 绑定到随机端口，让操作系统选择
	localUDPAddr, err := net.ResolveUDPAddr("udp", ":0")
	if err != nil {
		serverTCPConn.Close()
		return nil, fmt.Errorf("resolve local UDP addr error: %w", err)
	}
	localUDPConn, err := net.ListenUDP("udp", localUDPAddr)
	if err != nil {
		serverTCPConn.Close()
		return nil, fmt.Errorf("listen local UDP error: %w", err)
	}

	udpClient := &Socks5UDPClient{
		Client:              c,
		serverTCPConn:       serverTCPConn,
		localUDPConn:        *localUDPConn, // 注意这里是值拷贝，因为 net.UDPConn 是结构体
		socks5ServerUDPAddr: &net.UDPAddr{IP: bindAddr.IP, Port: bindAddr.Port},
	}

	// Context for managing UDP forwarding goroutine lifecycle
	udpClient.ctx, udpClient.cancel = context.WithCancel(context.Background())

	// 启动一个 goroutine 监听 TCP 控制连接的关闭，以便关闭 UDP 关联
	go func() {
		defer func() {
			if r := recover(); r != nil {
				log.Printf("Recovered from panic in TCP control connection listener: %v", r)
		...
		// io.Copy(io.Discard, serverTCPConn) 阻塞直到 TCP 连接关闭或出错
		_, err := io.Copy(io.Discard, serverTCPConn)
		if err != nil && err != io.EOF {
			log.Printf("SOCKS5 UDP ASSOCIATE control TCP connection error: %v", err)
		}
		log.Printf("SOCKS5 UDP ASSOCIATE control TCP connection closed. Canceling UDP forwarding.")
		udpClient.Close() // 当 TCP 控制连接关闭时，关闭整个 UDP 客户端关联
	}()

	//log.Printf("SOCKS5 UDP ASSOCIATE client ready. Send UDP data to %s", udpClient.localUDPConn.LocalAddr())
	return udpClient, nil
}

// UDP 客户端的 Close 方法，用于关闭 TCP 控制连接和本地 UDP socket
func (u *Socks5UDPClient) Close() error {
	u.once.Do(func() {
		if u.cancel != nil {
			u.cancel() // 发送取消信号给 UDP 转发 goroutine
		}
		if u.serverTCPConn != nil {
			u.serverTCPConn.Close() // 关闭 TCP 控制连接
		}
		if u.localUDPConn.LocalAddr() != nil { // 检查是否已初始化
			u.localUDPConn.Close() // 关闭本地 UDP socket
		}
		u.wg.Wait() // 等待所有 UDP 转发 goroutine 真正退出
		//log.Println("Socks5UDPClient closed.")
	})
	return nil
}

// WriteToUDP 方法用于向 SOCKS5 服务器发送封装后的 UDP 数据包
// 应用程序应该调用此方法，而不是直接调用 localUDPConn.WriteToUDP
// targetAddr 是实际的 UDP 目标地址，packetData 是原始的 UDP 数据
func (u *Socks5UDPClient) WriteToUDP(packetData []byte, targetAddr net.Addr) (int, error) {
	if u.localUDPConn.LocalAddr() == nil {
		return 0, fmt.Errorf("Socks5UDPClient is closed or not initialized")
	}

	host, portStr, err := net.SplitHostPort(targetAddr.String())
	if err != nil {
		return 0, fmt.Errorf("invalid target address: %w", err)
	}
	port, err := strconv.Atoi(portStr)
	...
	// 完整发送到 SOCKS5 服务器 UDP 端口的数据包
	fullUdpPacket := append(socks5UdpHeader, packetData...)

	// 将封装好的数据包发送到 SOCKS5 服务器的 UDP 绑定地址
	_, err = u.localUDPConn.WriteToUDP(fullUdpPacket, u.socks5ServerUDPAddr)
	if err != nil {
		return 0, fmt.Errorf("send SOCKS5 UDP packet to server error: %w", err)
	}
	// 返回写入的原始数据包长度，而不是整个封装后的长度
	return len(packetData), nil
}

// ReadFromUDP 方法用于从 SOCKS5 服务器接收封装后的 UDP 数据包，并返回原始数据和源地址
// 应用程序应该调用此方法，而不是直接调用 localUDPConn.ReadFromUDP
func (u *Socks5UDPClient) ReadFromUDP(p []byte) (n int, addr net.Addr, err error) {
	if u.localUDPConn.LocalAddr() == nil {
		return 0, nil, fmt.Errorf("Socks5UDPClient is closed or not initialized")
	}

	respBuf := make([]byte, 65535)                       // Buffer to receive full SOCKS5 UDP packet
	nResp, _, err := u.localUDPConn.ReadFromUDP(respBuf) // Read from local UDP socket
	if err != nil {
		return 0, nil, fmt.Errorf("read from local UDP error: %w", err)
	}
	...
	case ATYP_DOMAINNAME:
		...
		// SOCKS5 协议规定 BND.ADDR 可能是域名，但实际上代理服务器通常会返回 IP。
		// 这里尝试解析为 IP，如果不是，就保留为字符串。
		parsedIP := net.ParseIP(string(respBuf[responseOffset : responseOffset+domainLen]))
		if parsedIP != nil {
			sourceIP = parsedIP
		} else {
			//log.Printf("Warning: SOCKS5 UDP response BND.ADDR is a domain: %s. Returning as string.", string(respBuf[responseOffset:responseOffset+domainLen]))
			// 如果返回域名，这里需要处理成 net.UDPAddr。
			// 简单起见，如果不是 IP，就创建空的 IP，实际应用可能需要更复杂的域名解析回传。
			sourceIP = net.ParseIP("0.0.0.0") // Fallback
		}
		responseOffset += domainLen
	default:
	...
	copy(p, respBuf[responseOffset:responseOffset+dataLen])

	// 返回原始数据长度和源地址
	return dataLen, &net.UDPAddr{IP: sourceIP, Port: sourcePort}, nil
}
